...
Unprotect[Geass`Private`Package`$TopLevelLoad];
Geass`Private`Package`$TopLevelLoad=
...
$PackageContexts={
		"Geass`",
		"Geass`Private`Package`",
		"Geass`Private`Hidden`"
		};
$PackageDeclared=
...
PackageScopeBlock[e_,scope_String:"Hidden"]/;TrueQ[$AllowPackageRescoping]:=
	With[{newcont="Geass`Private`"<>StringTrim[scope,"`"]<>"`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx="Geass`Private`"<>StringTrim[scope,"`"]<>"`"
		},
		Replace[names,
...
If[(Clear@Geass`Private`Package`$loadAbort;!#)&@Geass`Private`Package`$loadAbort,
	Geass`Private`Package`PackagePostProcessContextPathReassign[]
	]