...
TriangleForm[triArray_List, OptionsPattern[]] := Block[
	{n = Length[triArray]},
	Graphics[MapIndexed[
		Text[
			Style[#1, Large, OptionValue[ColorFunction][(Min[triArray] - #1) / Subtract @@ MinMax[triArray]]],
			{Sqrt[3] * (n - 1 + #2.{-1, 2}), 3 * (n - First[#2] + 1)} / 2
		]&,
		triArray, {2}]
	]
];
...
ColorForm[expr_, OptionsPattern[]] := Block[
	{digit, colored},
	digit = (ToString[#] -> "\*\n" <> ToString@ToBoxes@Style[#, ColorData[OptionValue[Color]][If[NumberQ[OptionValue[Color]], #, # / 10]]])&;
	colored = DisplayForm[ToBoxes[expr, OptionValue[Form]] /. s_String :> With[
		{x = Quiet@ToExpression@s},
		RowBox@List@StringReplace[ToBoxes@s, digit /@ Range[0, 9]] /; MatchQ[x, _Real | _Integer]
	]];
