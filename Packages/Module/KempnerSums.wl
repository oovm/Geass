...
(* enter with a list of strings {s1, s2, ...}, like { "12", "345" }; return the T matrix *)
...
(* in this version, we input a T matrix, not a string *)
...
(* in this version, we input a T matrix *)
...
(* partial sum calculation, using previous KempnerSum *)
...
(* partial sum calculation, based on previous KempnerSum *)
...
(* given the input list, return the matrix A *)
	{ lst },
	lst = convertInputListToStrings[inputList];
	If[Length[lst] == 0, Return[ {{0}} ] ];
...
(* given the input string s, return the matrix A *)
	{ stringList = { s } },
	kSumGetT[stringList, iBase]
] ;   (* end of kSumGetT[string] *)

kSumGetT[i_Integer, iBase_ : 10] := Block[
(* given the input integer, return the matrix A *)
	{ stringList = { ToString[i] } },
	kSumGetT[stringList, iBase]
