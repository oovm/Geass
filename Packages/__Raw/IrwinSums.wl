...
This means that the sum can be made as large as desired by adding enough terms.
Suppose we delete from this series all terms with a "9" in the denominator.
That is, we remove 1/9, 1/19, 1/29, etc. Then the remaining series converges to a sum less than 80.
This was proved in 1914 by A. J. Kempner.*)
(*In 1916, Irwin proved that the 对1/n求和 where n has at most a fixed number of occurrences of one or more digits, is a convergent series.
This is a generalization of Kempner's result: in Kempner's series, the digit 9 occurs zero times.
It also follows that the 对1/n求和 where n has exactly n1 occurrences of digit d1, n2 occurrences of d2, etc., converges.*)
...
(* :Examples:
IrwinSum[9, 0] = 对1/n求和 where n has no 9's = 22.920676619264150 .
IrwinSum[9, 1] = 对1/n求和 where n has exactly one 9 = 23.044287080747848 .
IrwinSum[{9, 3}, {2, 0}] = 对1/n求和 where n has two 9's and no 3's = 2.593253652747189 .
...
sjkPrev;
cumulativeSums1;
(* this is a small array, size = countList[[1]] + 1 *)
iSumPrintLevel = 1;
...
power[x_, y_] :=
	Power[x, y];
(* in other cases, return same as built-in function *)
Clear[isThisASpecialDigit];
isThisASpecialDigit[iDigit_, nConditions_, digitList_List] :=
	Block[
	(* if iDigit is in the list of special digits, then return the 1-based location in the list; otherwise, return 0. *) { j },
...
bn[iBase_Integer, n_Integer, nConditions_Integer, digitList_?VectorQ] :=
	Block[
	(* this is for a set of special digitscompute sum(k = 0 through iBase-1) of k^n (for k not a special digit). example: if iBase = 10, one special digit (d),compute bn = 9 (n = 0), bn = 0^n + 1^n + ... + 9^n - d^n (n > 0).*) { bn = 0, k },
		If[ n == 0,
...
printAllSums1[iDigit_Integer, nDec_Integer, nConditions_Integer, countList_?VectorQ, iFormatted_Integer] :=
	Block[
	(* if there is just one condition, then printthe sum for k occurrences, k = 0, 1, 2, ... . *) { k },
		If[ nConditions != 1,
			Return[]
		];
		If[ iDigit > 0,
	...
	(* return this value *)
		...
		(* clear all counts *)
			...
			For[k = 1, k <= nConditions, k++, If[ kOccurFound[[k]] > countList[[k]],
				iOK = 0;
				(* this (i) had too many occurrences of one or more digits *)
				Goto[nextI];
			(* go to next iteration of main (i) loop *)
			];
			...
			(* end for k loop *)
			If[ iMatch == 1,
			(* (i) has n1 occurrences of digit d1, n2 of d2, etc. *)
				nTerms = nTerms + 1
			];
			xNumb = i;
...
computeMaxPowerNeeded[iBase_Integer, nDecimals_Integer, dsDigits_Integer] :=
	Block[
	(* enter with nDecimals, which specifies the desired tolerance, and dsDigits = 2nd parameter to directSummation.let a = iBase^(dsDigits - 1), b = iBase^dsDigits - 1, and eps = 10^-nDecimals.directSummation computes Sum[1/n^j, {n, a, b}] for j = 1, 2, ..., k.we want to estimate the value of k that makes Sum[1/n^k, {n, a, b}] < eps .in bases 2 through 10, a good approximation is Log[iBase, 10] * nDecimals/(dsDigits - 1).if this is not close to a value that makes the sum < eps, then we must solve an equation. we estimate the sum by an integral:Integrate[x^-c, {x, a, b}] = (b^(1-c) - a^(1-c))/(1-c). then we find the value of c that makes this < eps.*) { eps, a, b, r0, k, ns, c, r },
		eps = 10^-nDecimals;
		(* maximum tolerance *)

		(* a and b are the lower and upper limits in directSummation *)
		a = iBase^(dsDigits - 1);
		b = iBase^dsDigits - 1;
		...
		(* end for k loop *)
		Return[ -r0 ];
		(* return this negative value to indicate an error *)
		Label[loopDone];
		r0 + 2
	(* add 2 more powers just to be safe *)
	];
...
computeIrwinSum[iBase_Integer, digitList_?VectorQ, countList_?VectorQ, nDecimals0_Integer, iFormatted_Integer,nDigits_:0, threshold_:-1 ] :=
	...
	(* sumSmallerK is the cumulative sum over all 0 <= k1 <= n1, 0 <= k2 <= n2, etc. *)
		...
		(* if nConditions = 1 and countList[[1]] = (say) 434, maxDigits needs to be fairly large *)
		If[ Max[countList] > 10,
		...
		If[ maxJ < 0,
			maxJ = -maxJ;
			(* change back to positive, then print it *)
		...
		jMaxPower = maxJ;
		(* this value depends on nDecimals *)
		...
		(* outermost loop *)
		(* compute the sum of numbers in the set that have have (iDigit) digits,using the sums of reciprocals of powers over numbers having (iDigit-1) digits*)
			maxTermAddedI = 0;
			(* largest term added for this value of iDigit *)
			maxTermArray = Table[ 0, { jMaxPower } ];
			(* set maxTermArray to all 0's *)
			For[jPower = 1, jPower <= jMaxPower, jPower++, nMax = jMaxPower - jPower;
			maxTermAddedJ = 0;
			(* largest term added for this value of jPower *)
			...
			(* for this index value, set kList = corresponding n-tuple. *)
				...
				(* this is for { 10 * S(i, k1, k2, ..., k(k)-1, ..., kn) + d(k) } *)
					...
					kList[[k]] = kList[[k]] + 1;
				(* restore the kth index value *)
				];
				(* end if kList[k] *)
				];
				(* end For k loop *)
				(* this is for the final set in the above union{ 10 * S(i, k1, k2, ..., kn) + d. where d is none of d1, d2, ..., dn }*)
			...
			If[ (nDigits > 0) && (nDigits == iDigit),
			(* request is to compute partial sums *)
				iDone = 1;
				If[ iPrint >= 1,
					If[ iBase == 10,
						Print["partial sum through ", iDigit, " digits = ", requestedSum],
			...
			If[ nDigits > 0,
				Continue[];
			(* if calculating partial sums only, skip checks for convergence *)
			];
			(* if all sjk values are tiny, then we are done *)
			iAllTiny = 1;
		...
		(* end For iDigit loop *)
		Label[endLoops];
		(* return just the value of the requested sum *)
...
iPartialSumThreshold[digitList_?VectorQ, countList_?VectorQ, threshold_?Positive, nDecimals_:15, iBase_:10] :=
	...
	(* example of "list" version of iPartialSum: the 对1/n求和 where n has three 9's and one 0, is IrwinSum[{9, 0}, {3, 1}] = 2.888545932755274 . therefore, the threshold must be less thanthisvalue. in this example, we take the threshold to be 2. iPartialSumThreshold[ { 9, 0 }, { 3, 1 }, 2 ]= {27, 1.910422503190251, 28, 2.0043388417551473}*) { iFormatted = 0, nDigits = 0, totalSum, iPrintLevelSave,xSum1 = 0, nDig1 = 0, xSum2, nDig2, errorReturn = {-1, -1, -1, -1}, tAcc, nDec2,errStr = "Use backquote notation iPartialSumThreshold[ digit, count, threshold``nDecimals]],\or enclose the threshold in double quotes"},
		...
		iSumPrintLevel = -1;
		(* suppress all printed output except error messages *)
		...
		(* xSum1 should be less than the threshold.if it is not, decrease the number of digits by 1 *)
			nDig2 = nDig1;
			xSum2 = xSum1;
			nDig1 = nDig2 - 1;
			xSum1 = iPartialSum[digitList, countList, nDig1, nDec2, iBase];
		...
		iSumPrintLevel = iPrintLevelSave;
		(* if the computed xSum1 and xSum2 are correct, thenxSum1 must be < threshold and xSum2 must be >= threshold. *)
		If[ ! (xSum1 < threshold),
...
iPartialSumThreshold[digitList_?VectorQ, countList_?VectorQ, pSumStr_String, nDecimals_:15, iBase_:10] :=
	...
	(* the threshold was entered as a string. add double backquotes to specify the accuracy, then call another version of iPartialSumThreshold. *) { pSum, inputStr2, decPtList, quoteList, nDecimalsInput, nDec2, errorReturn = { -1, -1, -1, -1 }},
		...
		(* get list of starting, ending positions *)
		quoteList = StringPosition[pSumStr, "`"];
		If[ (Length[decPtList] == 1) && (Length[quoteList] == 0),
		(* input has one decimal point and no backquotes (usual case); number of decimals = number of chars between decimal point and end of string. append backquotes and a number to specify the accuracy of the input,then convert the new string to a floating-point number with that accuracy.*)
			nDecimalsInput = StringLength[pSumStr] - decPtList[[1]][[1]];
			(* nDec2 = Max[nDecimalsInput + 2, nDecimals]; *)
			(* from kempnerSums.m *)
		...
		If[ (pSum == $Failed) || (NumericQ[pSum] == False),
			Print["Invalid input"];
			(* cannot convert input to a numeric value *)
			Return[ errorReturn ]
		];
